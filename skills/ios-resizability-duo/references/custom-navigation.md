...
For a UIKit system tab container, inspect its default behavior before overriding it: `UITabBarController.Sidebar.preferredPlacement = .automatic` resolves to a tab bar on iOS, including Duo's regular-width context; a sidebar is opt-in. Avoid forcing `.tabBar` just to preserve that default, or forcing a tiled sidebar when its overlay behavior is appropriate. On a retained custom vertical bar, the 27.1 preferred-edge trait and `UIView.LayoutRegion.bar(onEdge:extent:)` with `layoutGuide(for:)` can reserve the bar's local space. Reproduce relevant system behaviors such as overflow, long-press labels, and Large Content Viewer support; use ordinary Auto Layout for tab accessories as they resize. [Tab/sidebar answer](https://developer.apple.com/forums/thread/847961), [custom bar answer](https://developer.apple.com/forums/thread/847835), [tab accessory answer](https://developer.apple.com/forums/thread/847807)
...
For a custom Back appearance, prefer the navigation bar's supported back-indicator customization so the system Back behavior remains intact. A wide segmented control is usually a poor vertical-bar item; prefer separate meaningful actions that can overflow when that preserves the interaction. UIKit `UIBarButtonItem.axisBehavior` is a 27.1 option for an appropriate item, not a way to make a whole custom destination selector into a system tab bar. [Back answer](https://developer.apple.com/forums/thread/847875), [segmented-control answer](https://developer.apple.com/forums/thread/847811)
