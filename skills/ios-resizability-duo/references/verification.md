...
| Paused or reversed fold | Layout follows current bounds and active reserved regions without a predicted transition order, stale debounce, or repeated measurement invalidation. |
| Arrangement collapses to one child | Essential content remains visible; controls in the hidden child have another accessible route. Verify actual child visibility rather than inferring it from size class or `splitArrangementAxis`. |
...
| UIKit tab/sidebar and retained vertical bars | Check default tab-bar placement before overrides; test opt-in sidebar overlay/tile choices, tab-accessory Auto Layout, overflow, long-press labels, Large Content Viewer, and system Back behavior. |
...
| Camera/accessory/hinge feature, if implemented | Camera direction and supported MultiCam sets are queried; virtual front-camera handover tolerates dropped frames and sample-timestamp discontinuity without an unnecessary session rebuild. Verify preview/shutter occlusion, AR continuity where relevant, and capability loss. Keep Simulator UI-flow results separate from hardware capture and performance evidence. |
